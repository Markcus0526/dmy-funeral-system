...
#import "FuWuRenYuanMingXiPiaoVC.h"
#import "MJRefresh.h"

@interface FuWuRenYuanCell()
@property (weak, nonatomic) IBOutlet UIView *vwFrame;
@property (weak, nonatomic) IBOutlet UILabel *lblBuyName;
...
@implementation FuWuRenYuanCell

-(void) setDepositLog:(STBonusDetailInfo*)log
{
	borderWidthColor(_vwFrame, 1, [UIColor darkGrayColor]);
	
	_lblBuyName.text = log.user_name;
	_lblTombID.text = log.tomb_no;
	_lblDatetime.text = log.buy_time;
	_lblPrice.text = log.bonus_desc;
}

@end

@interface FuWuRenYuanMingXiPiaoVC ()
{
...
@implementation FuWuRenYuanMingXiPiaoVC
...
- (void) callGetBonusList : (bool) bonus_type pageno:(int)pageno
{
	...
	[[[CommManager getCommMgr] comSvcMgr] GetBonusDetailList:[Common userInfo].uid bonus_type:bonus_type page_no:pageno];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	FuWuRenYuanCell * cell = [tableView dequeueReusableCellWithIdentifier:szID];
