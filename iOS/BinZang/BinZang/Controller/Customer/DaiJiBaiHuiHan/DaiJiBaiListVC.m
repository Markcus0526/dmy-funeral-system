...
#import "DaiJiBaiListVC.h"
#import "DaiJiBaiDetailVC.h"
#import "MJRefresh.h"

@implementation DaiJiBaiTableCell
@end

@interface DaiJiBaiListVC ()
{
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
	...
	if ([segue.identifier isEqualToString:SEGUE_TO_DETAIL])
	{
		...
		STDeputyLog * datainfo = (STDeputyLog *)[arrItems objectAtIndex:button.tag];
		// set data
		DaiJiBaiDetailVC * destCtrl = (DaiJiBaiDetailVC *)segue.destinationViewController;
		destCtrl.mDeputyInfo = datainfo;
	}
}
...
- (void) initControls
{
	...
	[self callGetDeputyList:nCurPageNo];
}
...
/**
 * call get deputy list service
 */
- (void) callGetDeputyList : (int)pageno
{
	TEST_NETWORK_RETURN;
	
	[[CommManager getCommMgr] comSvcMgr].delegate = self;
	[[[CommManager getCommMgr] comSvcMgr] GetDeputyLogs:pageno];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	DaiJiBaiTableCell * cell = [tableView dequeueReusableCellWithIdentifier:szID];
	
	STDeputyLog * datainfo = (STDeputyLog *)[arrItems objectAtIndex:indexPath.row];
	[cell.lblTime setText:datainfo.time];
