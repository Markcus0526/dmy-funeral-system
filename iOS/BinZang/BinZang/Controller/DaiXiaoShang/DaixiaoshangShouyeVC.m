...
@interface DaixiaoshangShouyeVC ()
{
...
@property (weak, nonatomic) IBOutlet UILabel *lblNotifier;
@end

#define CHKBANNER_TIME_DELAY				60

@implementation DaixiaoshangShouyeVC
...
- (void) initControls
{
	...
	_lblNotifier.hidden = YES;
	roundRect(_lblNotifier, _lblNotifier.frame.size.width / 2);
...
- (void) getNewActCountResult		: (NSInteger)retcode retmsg:(NSString *)retmsg count:(int)count;
{
	if (retcode == SVCERR_SUCCESS) {
		...
		if (count > 0)
		{
			_lblNotifier.hidden = NO;
			_lblNotifier.text = [NSString stringWithFormat:@"%d", count];
		}else{
			_lblNotifier.hidden = YES;
		}
	}
...
- (IBAction)onTapActivities:(id)sender
{
...
- (IBAction)onTapZhangDan:(id)sender
{
...
- (IBAction)onTapFuWuRenYuan:(id)sender
{
	UIViewController *viewVC = [[UIStoryboard storyboardWithName:@"Main" bundle:nil] instantiateViewControllerWithIdentifier:@"FuWuRenYuanVC"];
	
	[self.navigationController pushViewController:viewVC animated:YES];
}

- (IBAction)onTapReserveZangYiShi:(id)sender
{
...
- (IBAction)onTapReserveZangPin:(id)sender
{
	YiShiYuYueVC *viewVC = [[UIStoryboard storyboardWithName:@"Main" bundle:nil] instantiateViewControllerWithIdentifier:@"YiShiYuYueVC"];
