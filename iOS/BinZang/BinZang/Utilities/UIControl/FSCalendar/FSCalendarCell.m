...
@property (strong,   nonatomic) UILabel *notifier;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
		...
		_notifier = [[UILabel alloc] init];
		_notifier.backgroundColor = [UIColor redColor];
		_notifier.textColor = [UIColor whiteColor];
		_notifier.textAlignment = NSTextAlignmentCenter;
		_notifier.hidden = NO;
		[self.contentView addSubview:_notifier];
    }
    return self;
}
...
- (void)setBounds:(CGRect)bounds
{
	...
	_titleLabel.font = [UIFont systemFontOfSize:(long)titleHeight - 4];
	...
	long noti_Size = (long)(_backgroundLayer.frame.size.height * 0.4);
	CGRect noti_frame = CGRectMake((long)([self fs_width] / 2 + 5), (long)([self fs_height] / 2 - eventSize*2 - (long)noti_Size), noti_Size, noti_Size);
	_notifier.frame = noti_frame;
	_notifier.layer.cornerRadius = noti_Size / 2.0;
	_notifier.clipsToBounds = YES;
}
...
- (void)configureCell
{
	...
	_notifier.hidden = _notifyCount <= 0;
	_notifier.text = [NSString stringWithFormat:@"%ld", _notifyCount];
    if ( _notifyCount == -3 ) // changed by rijw
    {
        _notifier.hidden = false;
        _notifier.text = @" ";
    }
}
