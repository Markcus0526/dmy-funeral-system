...
#define kDarkText   [UIColor colorWithRed:30/255.0  green:28/255.0  blue:17/255.0    alpha:1.0]
...
- (void)initialize
{
    ...
    _weekdayFont      = [UIFont systemFontOfSize:10];
    ...
    _titleColors[@(FSCalendarCellStateSelected)]    = kBlue;//[UIColor whiteColor];
	...
	_titleColors[@(FSCalendarCellStateToday)]       = kPink;//[UIColor whiteColor];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
	...
	cell.notifyCount		= [self hasNotifierForDate:cell.date];
...
- (long)hasNotifierForDate:(NSDate *)date
{
	if (_dataSource && [_dataSource respondsToSelector:@selector(calendar:hasNotifierForDate:)]) {
		return [_dataSource calendar:self hasNotifierForDate:date];
	}
	return NO;
}
...
- (void) gotoNextMonth
{
	NSDate *nextMonth = [_currentMonth fs_dateBySubtractingMonths:1];
	[self scrollToDate:nextMonth];
	_currentMonth = nextMonth;
}

- (void) gotoPrevMonth
{
	NSDate *prevMonth = [_currentMonth fs_dateByAddingMonths:1];
	[self scrollToDate:prevMonth];
