...
public class ResolutionUtility
{
	...
	public static ResolutionUtility instance = new ResolutionUtility();

	public ResolutionUtility() {}

	public static int getBaseWidth() { return BASE_WIDTH; }
	public static int getBaseHeight() { return BASE_HEIGHT; }


	public void iterateChild(View view, int screenWidth, int screenHeight)
	{
		if (screenWidth > 0 && screenHeight > 0)
			setResolution(screenWidth - 3, screenHeight - 3);
	...
	private void updateLayout(View pView)
	{
		...
		// setting with and height
		{
			if (pLayoutParams.width > 0)
			{
				...
				pLayoutParams.width = Math.max(1, (int)(nWidth * xDirRate + 0.50001));
			...
			if (pLayoutParams.height > 0)
			{
				...
				pLayoutParams.height = Math.max(1, (int)(nHeight * yDirRate + 0.50001));
				pView.setLayoutParams(pLayoutParams);
			}
	...
	/**
	 * Method to get screen size in pixels
	 *
	 * @param appContext
	 * @return
	 */
	public static Point getScreenSize(Context appContext) {
		...
		if (SysDevUtility.getSystemVersion(appContext) >= Build.VERSION_CODES.HONEYCOMB_MR2)
