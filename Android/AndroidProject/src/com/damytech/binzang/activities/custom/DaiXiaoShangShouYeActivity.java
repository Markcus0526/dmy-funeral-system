 ...
 * Date : 2015-04-14
 * Time : 00:07
 * File Name : DaiXiaoShangShouYeActivity
 */
public class DaiXiaoShangShouYeActivity extends SwitchAnimActivity {
	...
	/**
	 * DaiXiaoShang variables ************************************************
	 */
	private ArrayList<STAdvertImage> arr_daixiaoshang_adverts = new ArrayList<STAdvertImage>();
	...
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState, R.layout.activity_daixiaoshangshouye);
	}
	...
	protected void initializeActivity() {
		...
		String strTitle = getString(R.string.STR_TITLE_YEWUJINGLI) + "," + getString(R.string.STR_TITLE_NIHAO);
		...
		first_view = findViewById(R.id.daixiaoshang_layout);
		...
		second_advert_pager = (HorizontalPager)findViewById(R.id.daixiaoshang_advert_pager);
		second_indicator_layout = (LinearLayout)findViewById(R.id.daixiaoshang_page_indicator_layout);
	...
	private void onClickedDingJinDanChaXun() {
		pushNewActivityAnimated(DingJinDanChaXunDaiXiaoShangActivity.class);
	}

	private void onClickedFuWuRenYuan() {
		Bundle bundle = new Bundle();
		bundle.putBoolean(FuWuRenYuanActivity.EXTRA_DETAIL_TABLE, true);
		pushNewActivityAnimated(FuWuRenYuanActivity.class, bundle);
	}
	...
	private CommDelegate comm_delegate = new CommDelegate() {
		...
		public void getAdvertsResult(int retcode, String retmsg, ArrayList<STAdvertImage> arrImages, ArrayList<STRelative> arrRelatives) {
			...
			if (retcode == CommErrMgr.ERRCODE_NONE) {
				DaiXiaoShangShouYeActivity.this.arr_daixiaoshang_adverts.clear();
				DaiXiaoShangShouYeActivity.this.arr_daixiaoshang_adverts.addAll(arrImages);

				updateDaiXiaoShangImageGallery();
			}
		}
		...
		public void getNavDestinationResult(int retcode, String retmsg, double lat, double lng) {
			...
			if (retcode == CommErrMgr.ERRCODE_NONE) {
				BNavigatorActivity.startNavigationActivity(DaiXiaoShangShouYeActivity.this,
						lat,
						lng);
			} else {
				ToastUtility.showToast(DaiXiaoShangShouYeActivity.this, retmsg);
			}
		}
		...
		public void getBannerImagesResult(int retcode, String retmsg, ArrayList<STAdvertImage> arrImages) {
			...
			if (retcode == CommErrMgr.ERRCODE_NONE) {
				DaiXiaoShangShouYeActivity.this.arr_newuser_adverts.clear();
				DaiXiaoShangShouYeActivity.this.arr_newuser_adverts.addAll(arrImages);
	...
	private void updateDaiXiaoShangImageGallery() {
		second_indicator_layout.removeAllViews();
		arrSecondIndicatorViews.clear();

		for (int i = 0 ; i < arr_daixiaoshang_adverts.size(); i++) {
			STAdvertImage image = arr_daixiaoshang_adverts.get(i);
	...
	private void onClickedMapNav() {
		CommonAlertDialog dialog = new CommonAlertDialog.Builder(DaiXiaoShangShouYeActivity.this)
