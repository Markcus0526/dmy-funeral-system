...
namespace JpushApiClient
{
    class JPushApi
    {
        ...
        public static String app_key = "16ec2ed78399560789d2b44c";
        public static String master_secret = "d1fe440d0b4a0936516b68e3";

        public static void DeleteTag(String devtoken)
        {
            JPushClient client = new JPushClient(app_key, master_secret);
            try
            {
                client.deleteTag(devtoken, null);
            }
        ...
        public static void SendPushNotification(String devtoken, String content) 
        {
            Console.WriteLine("*****开始发送******");
            JPushClient client = new JPushClient(app_key, master_secret);
            //PushPayload payload = PushObject_All_All_Alert();
            PushPayload payload = PushObject_android_and_ios(devtoken, content);
        ...
        public static PushPayload PushObject_Android_Tag_AlertWithTitle(String devtoken, String content)
        {
            PushPayload pushPayload = new PushPayload();

            pushPayload.platform = Platform.android();
            pushPayload.audience = Audience.s_tag(devtoken);
            pushPayload.notification = Notification.android(content, "消息");

            return pushPayload;
        }

        public static PushPayload PushObject_android_and_ios(String devtoken, String content)
        {
            PushPayload pushPayload = new PushPayload();
            pushPayload.platform = Platform.android_ios();
            var audience = Audience.s_tag(devtoken);
            pushPayload.audience = audience;
            var notification = new Notification().setAlert(content);
            notification.AndroidNotification = new AndroidNotification().setTitle("消息");
