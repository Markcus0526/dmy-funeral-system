...
<div class="row" style=" min-width:1000px">
            <div id="responsive" class="modal fade" tabindex="-1" data-width="760">
                     <div class="modal-header">
							 ...
							 <h4 class="modal-title">代祭拜预约行事历导出</h4>
					 </div>
                     ...
                     <div class="modal-footer">
						...
						<button type="button" class="btn blue" onclick="exportjidetail()">确认</button>
					 </div>
                </div>
				<div class="col-md-12">
					<!-- BEGIN EXAMPLE TABLE PORTLET-->
                      <div style=" height:90px; margin-top:4%; border:1px solid #3B9C96">
                        ...
                        <input class="date-picker" id="etime" name="etime" type="text" data-date-format="yyyy-mm-dd" style="width:140px; height:28px" /> <div style=" margin-top:1%">
                         <label style="">是否取消：</label><input type="checkbox" checked="checked" id="sta1"/>未取消<input type="checkbox" checked="checked" id="sta2"/>已取消 <a id="searchdata" class="btn btn-sm btn-success" onclick="SearchDJiBai()" style=" margin-left:2%"><i class="fa fa-search"> </i>搜索</a><a id="A1" class="btn btn-sm btn-success" onclick="ExportJiBai()" style=" margin-left:2%"><i class="fa fa-mail-forward"> </i>导出代祭拜仪式</a><a class="btn btn-success btn-sm" data-toggle="modal" href="#responsive" style=" margin-left:2%"><i class="fa  fa-tasks"> </i>导出行事历</a></div>
                 </div>
					<div class="portlet box green-haze" style=" margin-top:5%">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-group"></i>代祭拜仪式预约
							</div>
   ...
   var url ="<%= ViewData["rootUri"] %>";
    jQuery(document).ready(function () {
        //QuickSidebar.init(); // init quick sidebar
             $(".select2").css('width', '50px').select2({ minimumResultsForSearch: -1 });
       initTable1 = function () {
            var table = $('#sample_1');
            ...
            var status = sta1+sta2;
             oTable = table.dataTable({
                ...
                "language": {
                 ...
                 "sServerMethod": "POST", 
                "sAjaxSource": url+"ShangPinOrder/GetDJBOrder",
                "aoColumns": [
                      ...
                      "fnServerData": function(sSource, aoData, fnCallback){
                                      $.ajax({
                                          ...
                                          "type": "POST",
                                          "url": sSource,
                                          "data":{"cname":clientname, "phone":clientphone,"stime":stime,"etime":etime,"status":status} ,
                                           "success": fnCallback
                                             } );
                      },     
                         "columnDefs": [
				        ...
				        },
				        {
				           ...
				           if (row.status==0)
				           {
                        return '<a onclick="ViewInfo('+data+')">商品订单</a> &nbsp;&nbsp;'+'<a onclick="CallBack('+data+')">回函</a>'+'&nbsp;&nbsp;<a onclick="DelDJibai('+data+')">取消</a>';
				           }
                            if (row.status==1||row.status==2||row.status==3)
				           {
                        return '<a onclick="ViewInfo('+data+')">商品订单</a>&nbsp;&nbsp;<a onclick="SeeCallBack('+data+')">查看回函</a>';
				           }
                                },
				            sClass: 'center'
				        }
      ...
      jQuery(function ($) {
          ...
          function SearchDJiBai(){

                     oTable.fnDestroy();
                      initTable1();
       }
                          function DelDJibai(uid){
                   // alert(uid);
                      var rootUri ="<%= ViewData["rootUri"] %>";
                         $.ajax({

                            url: rootUri + "ShangPinOrder/DelDJiBai",
                            data: {
                                "sid": uid
           ...
           function ViewInfo(uid){
            var rootUri ="<%= ViewData["rootUri"] %>";
             ...
             function CallBack(uid){
              var rootUri ="<%= ViewData["rootUri"] %>";
     		window.location.href=rootUri+"CallBack/Index?uid="+uid;
       

             }
              function SeeCallBack(uid){
                  var rootUri ="<%= ViewData["rootUri"] %>";
     		      window.location.href=rootUri+"CallBack/SeeCallBack?uid="+uid;
       

             }
           function ExportJiBai(){
           var rootUri ="<%= ViewData["rootUri"] %>";
           var clientname = $('#name').val();
            //alert(name);
            var clientphone = $('#phone').val();
            ...
            location.href= rootUri + "JiBaiReserve/ExportJi?cname="+clientname+"&&phone="+clientphone+"&&stime="+stime+"&&etime="+etime+"&status="+status;

           }
    function exportjidetail(){
          var rootUri ="<%= ViewData["rootUri"] %>";
          toastr.options = {
                                    "closeButton": false,
                                    "debug": true,
        ...
        toastr["error"]("请输入具体日期！");
                ...
                location.href= rootUri + "JiBaiReserve/ExportJiBai?extime="+extime;
                   }
